...
The collector uses bounded requests and does not attempt to bypass authentication, paywalls, robots restrictions, or anti-bot measures. Unavailable article bodies fall back to feed excerpts. The default source mix combines OpenAI, Hacker News, focused GitHub AI feeds, Google and Microsoft developer blogs, LangChain, and Cloudflare's agent/Workers AI changelogs. It also monitors releases from Aider, Anthropic Skills and Claude Cookbooks, LangGraph, the OpenAI Agents SDK and Codex, Microsoft Agent Framework, official MCP servers, and OpenHands. These code-first release feeds carry `topic_hints`, which lets versioned project updates be considered even when their titles do not contain a discovery keyword; Workers AI still makes the final relevance and tab decision from the release content.
